...
> **Update, 30 July 2026.** The integrity gaps this review identified in §2 ("The blocking flaw") have been closed by deletion rather than caveat: the synthetic-outcome backtest, the seeded expert-judgment family, the circular GPD fit, the illustrative counterparty table (which was feeding invented limit breaches into the real escalation queue), and the Alpha Vantage key path are all gone; the unfitted logistic no longer claims to emit a probability. The universe expanded to 87 companies and 97 commodity markets across 10 families, with a commodity reference sheet whose structure follows the Commodities 101 fact-sheet breakdown plus an India hedgeability layer — which produced a new analytical output, the **Hedgeability Matrix** (§4.3's "billable features" now has a front end). Supply concentration is computed from producer shares rather than hand-set, closing part of P1 item 15. See `README.md` for what changed and `BRAINSTORM.md` for the ranked build list that supersedes §6's P2–P4 ordering.

**Reviewed:** 16 July 2026 · commit `9f5ab4a` · build `2026-07-10-sankey6-v15` · live check of https://s-ganti.github.io/risk-radar/
**Status:** ✅ P0 (items 1–10) shipped 16 Jul 2026 · ✅ **P1 live-data backbone (items 11–18) shipped 17 Jul 2026, CI-verified through 21 Jul** — daily pipeline (`scripts/pipeline.py` + Actions cron) fetches ECB FX / EIA / IMF PCPS / LBMA series, computes EWMA vols, 105-pair correlation matrix, market regime, market velocities and real score history; dashboard consumes `data/latest.json` with as-of chips, ₹ crore impacts from verified FY26 financials, and a CBAM certificate-cost estimator. The Actions cron has now run unattended for 4 consecutive days (17–20 Jul); `data/history.json` holds real multi-point score history and the sparklines render live data (verified on the deployed site 21 Jul — e.g. the `oil` risk moved 69→60 as market velocity fell, while curated-velocity regulation risks held flat, exactly as designed). **Known feed limitations (documented, not blockers):** EIA Brent via FRED publishes with a ~1-week lag, so the "live" Brent cell is week-lagged EOD (correctly dated); EUA and MCX are manual/stub feeds pending P3 regwatch; exposure/concentration inputs remain curated (P2 computes them). Next: P2 real model calculations.
